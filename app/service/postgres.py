...
class PostgresOrm:
    ...
    async def get_memory(session: AsyncSession, memory_id: int) -> Iterable | None:
        data = await session.execute(select(Memory).where(Memory.id == memory_id))
        return data.scalars().first()

    @staticmethod
    async def set_memory(
        session: AsyncSession, current_user: User, memory: MemoryCreateRequest
    ) -> bool:
        photos = None
        if memory.photos:
            photos = [
                Photo(user_id=current_user.user_id, photo_url=str(photo))
    ...
    async def update_memory(
        session: AsyncSession, current_user: User, memory: MemoryUpdateRequest
    ):
        await session.execute(
            update(Memory)
            .where(
